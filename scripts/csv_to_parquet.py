...
file_path = STAGING_PATH + "usa_county_wise_parquet"
command = f"hdfs dfs -du -h {file_path}"
subprocess.run(command, shell=True)
...
end_time = time.time()
print(f'CSV read time {end_time - start_time} seconds')
...
df_usa_county_wise = spark.read.parquet(STAGING_PATH + "usa_county_wise_parquet").count()
df_worldometer_data = spark.read.parquet(STAGING_PATH + "worldometer_data_parquet").count()
end_time = time.time()
print(f'Parquet read time {end_time - start_time} seconds')
